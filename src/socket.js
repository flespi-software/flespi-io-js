...
const _events = {} /* store name of events and array of handlers by current event {name: [...handlers]} */
  ...
  const fallbackMessageProcessing = (topic, message, packet) => {
      const topicPath = topic.split('/'), /* getting full topic path */
        ...
        activeTopicsId = Object.keys(_topics).filter((checkedTopicId) => {
          const currentTopicPath = _topics[checkedTopicId].name.split('/')
...
mqttConnector.subscribe = async function subscribe (topic) {
  const isProtocolNew = _config.mqttSettings && _config.mqttSettings.protocolVersion === 5
  ...
  return topic.reduce(async (result, topic) => {
    ...
    if (_client) {
      try {
        const granted = await _client.subscribe(topic.name, topic.options)
        result[id] = granted
      } catch (e) {
        result[id] = e
      }
...
mqttConnector.unsubscribe = async function unsubscribe (name, unsubId, options) {
  ...
  const removableTopicsIndexes = Object.keys(_topics).reduce((result, topicId, index) => {
  ...
  if (needUnsubscribe && !_client._client.disconnecting) {
    const state = await _client.unsubscribe(name, options)
    return state
  } else { return false }
}
...
mqttConnector.unsubscribeAll = async function unsubscribeAll (options) {
  _timestampsByTopic = {}
  for (const topicId of Object.keys(_topics)) {
...
mqttConnector.publish = async function () {
  if (_client) {
    const state = await _client.publish(...arguments)
    return state
  } else {
